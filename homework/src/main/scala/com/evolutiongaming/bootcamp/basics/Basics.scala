package main.scala.com.evolutiongaming.bootcamp.basics
...
object Basics {
  ...
  string6 eq string7 // false - reference equality - are they the same object? (not the same in this example)
  ...
  val stringLength: String => Int = (s: String) => s.length /* implement here s.hashCode()*/
  ...
  // Question: What is `A` for `formatNamedDouble` in this `formattedLong` invocation of it? Long
...
formatNamedValue[List[String]](name = "List", _.mkString(", "))
